...
{   (!load&&!created) &&     
    <Box  sx={{  marginTop: 4,marginBottom:4,display: 'flex',flexDirection: 'column',alignItems: 'center',}}>
          ...
          <Controller name="title" control={control} defaultValue=''
            render={({ field }) => 
              (
                <TextField fullWidth {...field} label="Titulo"variant="outlined" error={!!errors.title} helperText={errors.title?.message}/>
              )}/>
          <Controller name="message" control={control} defaultValue='' 
          render={({ field }) => 
            (
            <TextField {...field} label="Escreva aqui o texto da  sua mensagem..." variant="outlined" multiline rows={16}  fullWidth error={!!errors.message} helperText={errors.message?.message} />
            )}/>
